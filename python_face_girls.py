...
#唯一必须填的信息就这三行
APP_ID = "24781855"
API_KEY = "3k5fBIcslVvlLgQeZqi99w3p"
SECRET_KEY = "a0jBXqzEua5BTNmbRuWNDHc3LubsqqjE"
# 文件存放目录名，相对于当前目录
DIR = "image"
# 过滤颜值阈值，存储空间大的请随意
BEAUTY_THRESHOLD = 45
...
def process_activities(datums, face_detective):
    for data in json.loads(datums, encoding="utf-8")["data"]:
        target = data["target"]
        ...
        for image in images:
            ...
            for score in scores:
                filename = ("%d--" %
                            score) + author_name + "--" + question_title + (
                                "--%d" % seq) + ".jpg"
